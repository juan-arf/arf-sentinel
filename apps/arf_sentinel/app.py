...
# Sidebar Auto-play
conf = st.sidebar.checkbox("Auto---play (5 sec/phase)", value=False)
if conf:
    st.sidebar.info("📥 Cinema mode active – sit back and watch the story unfold.")
...
if conf and st.session_state.phase < 8:
...
# Reusable components
def bayesian_explanation(prior_risk, evidence_conf, posterior_risk):
    ...
    st.markdown(f"""
    <div class="risk-bar">
        <div style="width:{posterior_risk*100}%; height:100%; background:linear-gradient(90deg, #00FF88, #e74c3c); border-radius:12px;"></div>
    </div>
...
# Phase 1 - CRAFT Investigation
    ...
    <div class="card fade-in">
        <span style="color:#00bcd4; font-size:1.2rem;">
            ✅ Schema discovered · ✅ Dependency graph mapped · ✅ Blast radius calculated
        </span>
    </div>
    """, unsafe_allow_html=True)
...
# Phase 3 - ARF Interception (animated battle bar)
    ...
    st.markdown('<p class="phase-title">🭡️ PHASE 3 · ARF GOVERNANCE INTERCEPTION</p>', unsafe_allow_html=True)
    state = st.session_state.state
    ...
    # Confidence vs Risk animated battle bar (HTML/CSS)
...
# Phase 4 - Bayesian Decision Engine
    ...
    bayesian_explanation(prior_risk, evidence_conf, posterior_risk)
...
# Phase 5 - Execution Boundary (Blocked)
...
# Phase 6 - Counterfactual Analysis
    ...
    bayesian_explanation(0.5, safe_dec.evidence_confidence, safe_dec.risk_probability)
...
# Phase 7 - Agent Re-Planning
    ...
    *Execution scope bounded by ARF.*
    """)
    if st.button("🏁 MAKE FINAL APPROVAL"):
...
# Phase 8 - Approval & Final
    ...
    st.markdown('<p class="phase-title">✅ PHASE 8   APPROVAL AND FINAL</p>', unsafe_allow_html=True)
    decision = st.session_state.arf_decision
    st.markdown(f"### ARF DECISION: <span style='color:#00FF88; text-shadow:0 0 10px #00FF88;'>APPROVE</span>")
    st.success("EXACTION AUTHORITY GRANTED")
    st.balloons()
    ...
    with col3:
        st.markdown("🔢 **ARF**\n*Authorize*")
    st.markdown("---")
    st.markdown("✅ Bayesian Risk Assessment  °'b�� Counterfactual Planning  ° ✅ Bounded Execution Authority")
